...
4. There is another uncertainty, that of not knowing that the
...
NOTE: plot the prediction surface to make sense of changed effects.
...
Try `visreg()` with `scale = "linear"` and `scale = "response"`
to see the difference between the linear predictor and the response scale.
...
Assess logistic regression model using ROC curve and AUC:

```{r}
table(y01 = x$y01, fit = ifelse(fitted(mB0) > 0.5, 1, 0))
table(y01 = x$y01, fit = ifelse(fitted(mB4) > 0.5, 1, 0))

pROC::auc(x$y01, fitted(mB0))
pROC::auc(x$y01, fitted(mB1))
pROC::auc(x$y01, fitted(mB4))

plot(pROC::roc(x$y01, fitted(mB0)), col = 4)
plot(pROC::roc(x$y01, fitted(mB1)), add = TRUE, col = 3)
plot(pROC::roc(x$y01, fitted(mB4)), add = TRUE, col = 2)
```
...
- The logic behind LM and GLM is nearly identical. Our goal is to
reduce the prediction uncertainty using auxiliary information.
- The main point is that we have to use appropriate statistical model
to quantify the intrinsic variability in the response variable.
- In both LM and GLM, we model the mean as a function of the
...
variable etc.
- Concept of residuals is extremely tricky when the range of the
