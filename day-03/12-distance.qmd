...
library(knitr)
draw_ellipse <- function(x, y, a = 1, b = 1, angle = 0, segment = NULL, arc.only = TRUE,
    ...
    if (n == 1) {
        draw1ellipse(x, y, a, b,
            angle = angle, segment = segment,
        ...
        lapply(1:n, function(i) {
            draw1ellipse(x[i], y[i], a[i],
                b[i],
                angle = angle[i], segment = segment[i, ], arc.only = arc.only,
...
The distance function ($g(d)$) describes the probability of detecting an individual given the distance between the observer and the individual ($d$). The detection itself is often triggered by visual or auditory cues, and thus depends on the individuals being available for detection  (and of course being present in the survey area).
...
```{r}
dd <- ds(dt$d,
    truncation = rmax, transect = "point",
...
m <- 2
plot(0,
    type = "n", xlim = m * c(-rmax, rmax), ylim = c(-w, 1 + w),
    axes = FALSE, ann = FALSE
)
yh <- g(rmax, tau = tau)
lines(
    seq(-rmax, rmax, rmax / 100),
...
round(rbind(
    ...
    D = c(Den,
        unlimited = mean(rowSums(Ydis1)) / Apq1,
        truncated = mean(rowSums(Ydis2)) / Apq2
    )
), 4)
...
Ddur <- matrix(c(3, 5, 10), nrow(Ydur), 3,
    byrow = TRUE,
...
Ddis <- matrix(c(0.5, 1, Inf), nrow(Ydis), 3,
    byrow = TRUE,
...
yall_dis <- Xtab(
    ~ SiteID + Dis + SpeciesID,
    detect::josm$counts[detect::josm$counts$DetectType1 != "V", ]
)
...
# Constant
Mmulti1 <- detect::cmulti(
    Ystack | Dstack ~ spp - 1, Xstack,
    type = "dis"
)
summary(Mmulti1)

Mmulti2 <- detect::cmulti(
    Ystack | Dstack ~ logmass + MaxFreqkHz + Mig2 + Hab2,
    Xstack,
    type = "dis"
)
summary(Mmulti2)
