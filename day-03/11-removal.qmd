...
plot(stepfun(v1$t, (0:nrow(v1)) / nrow(v1)),
    do.points = FALSE, xlim = c(0, 10),
...
legend("bottomright",
    bty = "n", lty = c(1, 1, 1, 1), col = c(1, 2, 4, 3),
...
D <- matrix(c(3, 5, 10), nrow(Y), 3,
    byrow = TRUE,
    dimnames = dimnames(Y)
)
X <- detect::josm$surveys[rownames(Y), c("DAY", "TSSR")]
head(Y[rowSums(Y) > 0, ])
head(D)
summary(X)
```
...
```{r}
curve(1 - exp(-x * phi_Me0),
    xlim = c(0, 10), ylim = c(0, 1), col = 4,
