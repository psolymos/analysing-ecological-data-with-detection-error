---
title: "Estimating and applying detectability offsets"
...
Note: to install INLA, you have to use:
...
op <- par(mfrow = c(1, 2))
matplot(Dec, Fit1,
    type = "l", ylim = c(0, max(Fit1, Fit2)),
    col = rep(1:3, each = 3), lty = rep(1:3, 3)
)
legend("topleft",
    bty = "n", col = c(1, 1, 1, 1, 2, 3), lty = c(1, 2, 3, 1, 1, 1),
...
## Poisson-Lognormal GLMM in INLA

See a tutorial [here](https://www.precision-analytics.ca/articles/a-gentle-inla-tutorial/)

```{r}
MPLNI <- inla(y ~ Decid + f(SiteID, model = "iid"),
    data = x, family = "poisson"
)

DPLNI <- drop(exp(X %*% MPLNI$summary.fixed[, "mean"]))
```
...
xi <- data.frame(
    y = y,
    x[, c(
        "Open", "Water", "Agr", "UrbInd", "SoftLin", "Roads",
...
pr1 <- t(apply(
    exp(Xpred2 %*% t(COEF1)), 1,
    quantile, c(0.05, 0.95)
))
pr2 <- t(apply(
    exp(Xpred2 %*% t(COEF2)), 1,
    quantile, c(0.05, 0.95)
))
