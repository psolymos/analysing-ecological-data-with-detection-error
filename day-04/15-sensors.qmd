---
title: "Analysing data from audio recordings and camera traps"
...
When combining human and ARU based counts, it is important that we understand how the two types of counts relate to each other.
There are different approaches to make them comparable, especially with respect to the sensitivity (boosting) of the signal from ARUs.
Signal strength (sound pressure levels) from ARUs relate to distance, this it can be used
to make sure the differences are understood.

- Triangulation methods: using time-synced ARU grids to track vocalization events in space and time, use this to build a relationship between distance and sound pressure levels
- Paired sampling: simultaneous sampling by human observers and ARUs at the same locations
- Distance calibration: measuring distances in the field and recoding the vocalization events, so a model between the field based distance and sound pressure levels can be established
- Playback experiments: broadcasting known signals at known distances and recording them by ARUs to establish the relationship between sound pressure levels and distance
...
SPP <- SPP[!(SPP %in%
    c(
        "CANG", "COLO", "COGO", "COME", "FRGU", "BCFR", "UNKN", "RESQ",
...
X$SurveyType <- factor(X$SurveyType, c("HUM", "ARU"))
...
for (spp in SPP) {
    ...
    Delta[spp, ] <- c(
        sqrt(exp(coef(mod1)["SurveyTypeARU"])),
...
`prop_open` 0 vs. 1 leads to different deviation from the 1:1 line, explain why.
