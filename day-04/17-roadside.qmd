---
title: "Dealing with roadside and other biases"
...
est_fun <- function(x, type = 1) {
    ...
    q <- if (is.infinite(max(rb))) {
        ...
        (tauhat^2 / max(rb)^2) * (1 - exp(-(max(rb) / tauhat)^2))
    }
...
p1 <- d[d$protocol == "10/Inf" & d$movement == 0, ] |>
    ...
    labs(
        subtitle = "10 min/Unlimited", fill = "cue rate (edge)",
...
p2 <- d[d$protocol == "3/1.5" & d$movement == 0, ] |>
    ...
    scale_fill_grey(start = 0.5, end = 1) +
    labs(
        subtitle = "3 min/150 m", fill = "cue rate (edge)",
        x = "detection radius (road)", y = "estimated density"
    ) +
    theme_minimal() +
    theme(legend.position = "none")
