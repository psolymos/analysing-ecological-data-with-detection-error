---
title: "Multi-species models and using species traits and phylogeny"
...
From
<https://github.com/psolymos/lhreg/blob/master/vignettes/lhreg.Rmd>

# Introduction

The present document can be viewed as `vignette(topic = "lhreg", package = "lhreg")`.
To install the package, use `remotes::install_github("psolymos/lhreg")`.
...
legend("topright",
    bty = "n", pch = c(21, 21, 22, 22), col = c(1, 2, 1, 2),
    legend = c(
        "Migratory/Closed", "Resident/Closed",
...
We averaged the 1000 pseudo-posterior trees from Jetz et al.
(2012) with Ericson backbone to represent the phylogenetic relationships among the species.
These were turned into the phylogenetic correlation matrix, `cor_matrix`.
...
mt0 <- lm(y ~ 1)
summary(mt0) # null model for DD
summary(mt) # full model for DD
## evaluating interactions
...
tM00 <- lhreg(
    Y = x$logtau, X = X0, SE = x$logtauSE, V = vc, lambda = 0,
    hessian = TRUE, method = met
)
tMl0 <- lhreg(
    Y = x$logtau, X = X0, SE = x$logtauSE, V = vc, lambda = NA,
    hessian = TRUE, method = met
)
tM0b <- lhreg(
    Y = x$logtau, X = Xt, SE = x$logtauSE, V = vc, lambda = 0,
    hessian = TRUE, method = met
)
tMlb <- lhreg(
    Y = x$logtau, X = Xt, SE = x$logtauSE, V = vc, lambda = NA,
    hessian = TRUE, method = met
)
...
pM00 <- lhreg(
    Y = x$logphi, X = X0, SE = x$logphiSE, V = vc, lambda = 0,
    hessian = TRUE, method = met
)
pMl0 <- lhreg(
    Y = x$logphi, X = X0, SE = x$logphiSE, V = vc, lambda = NA,
    hessian = TRUE, method = met
)
pM0b <- lhreg(
    Y = x$logphi, X = Xp, SE = x$logphiSE, V = vc, lambda = 0,
    hessian = TRUE, method = met
)
pMlb <- lhreg(
    Y = x$logphi, X = Xp, SE = x$logphiSE, V = vc, lambda = NA,
    hessian = TRUE, method = met
)
```
...
save(
    list = c(
        "cor_matrix", "lam", "lhreg_data", "met", "n",
        "vc", "x", "X0", "Xp", "Xt",
        "amp", "mp", "mp0", "mpx", "mpx2",
        "amt", "mt", "mt0", "mtx", "mtx2",
        "pM00", "pM0b", "pMl0", "pMlb",
...
## red-yl-blue
Col <- c(
    "#2C7BB6", "#6BAACF", "#ABD9E9", "#D4ECD3", "#FFFFBF", "#FED690",
...
sf <- function(z, loo, pb, type = c("wald", "jack", "boot"), level = 0.95) {
    ...
    if (type == "wald") {
        ...
        out <- structure(sapply(1:nrow(zz), function(i) {
            paste0(
                round(zz[i, 1], dig), " (SE +/- ", round(zz[i, 2], dig),
                pcut(zz[i, 4]), ")"
            )
        }), names = rownames(zz))
    }
    if (type == "jack") {
        CI <- t(apply(
            rbind(zz[, 1], loo[, 3:ncol(loo), drop = FALSE]), 2,
            quantile, a
        ))
        out <- structure(sapply(1:nrow(zz), function(i) {
            paste0(
                round(zz[i, 1], dig), " (", round(CI[i, 1], dig), ", ",
                round(CI[i, 2], dig), ")"
            )
        }), names = rownames(zz))
    }
...
cbind(aict, t(sapply(
    list(tM00, tMl0, tM0b, tMlb),
    function(z) z$summary[c("sigma", "lambda"), 1]
)))

cbind(aicp, t(sapply(
    list(pM00, pMl0, pM0b, pMlb),
    function(z) z$summary[c("sigma", "lambda"), 1]
)))
...
m1 <- rbind(m1,
    df = aicp$df, dAIC = round(aicp$dAIC, 3),
    XV_MSE = round(MSEp, 3)
)
m2 <- rbind(m2,
    df = aict$df, dAIC = round(aict$dAIC, 3),
    XV_MSE = round(MSEt, 3)
)
...
plot(lam, yv,
    type = "n", lwd = 1, ylim = exp(c(2 * Crit, 0)),
...
box()
legend(0.1, 1,
    bty = "n", border = c(Col1, Col3), fill = c(Col2, Col4), pt.cex = 2, pt.lwd = 2,
...
box()
legend("topleft",
    pch = 21, col = c(Col1, Col3), pt.cex = c(2, 2), bty = "n",
